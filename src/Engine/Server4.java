...
public class Server4 {
	...
	private String server3IPAddress;
	private int server3PortNumber;
	private int localIndex = 3;

	public Server4(int portNumber, String server3IPAddress, int server3PortNumber) {

		this.portNumber = portNumber;
		this.server3IPAddress = server3IPAddress;
		this.server3PortNumber = server3PortNumber;
	...
	public void startRunning() {
		...
		try {
			...
			showMessage("\nAttemping to connect to Server (3)... \n");
			server3Socket = new Socket(InetAddress.getByName(server3IPAddress), server3PortNumber);
			initiateServer3Threads();
	...
	private void initiateServer3Threads() throws IOException {

		receiveFromServer3 = new ReceiveFromClientThread(server3Socket);
		receiveFromServer3Thread = new Thread(receiveFromServer3);
		receiveFromServer3Thread.setName("Server 3 receptionThread");
		receiveFromServer3Thread.start();

		sendToServer3 = new SendToClientThread(server3Socket);
		sendToServer3Thread = new Thread(sendToServer3);
		sendToServer3Thread.setName("Server 3 transmissionThread");
		sendToServer3Thread.start();
	...
	private void broadcastToJoinedClients(String message) {
		...
		for (int i = 3; i < globalMemberList.size(); i += 4) {
						...
						if (!message.equals("")) {
							...
							if (message.charAt(0) == '$') {
								...
								if (commandArray[0].equals("$000")) {
									...
									sendMessageToServer3("$001 " + clientID + " server identification");
